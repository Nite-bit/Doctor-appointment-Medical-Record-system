...
?>
    ...
    if($_GET){
        ...
        if($action=='drop'){
            ...
            $sqlmain= "select sname from specialties where id='?"; // **Potential bug here: Extra ' before ?**
            $stmt = $database->prepare($sqlmain);
            $stmt->bind_param("s",$spe);
            $stmt->execute();
            $result = $stmt->get_result();

            // **Potential bug here: $spcil_res is not defined, should likely use $result from above**
            $spcil_array= $spcil_res->fetch_assoc();
            $spcil_name=$spcil_array["sname"];
            $nic=$row['docnic'];
            $tele=$row['doctel'];
