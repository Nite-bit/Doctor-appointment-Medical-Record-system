...
<body>
    ...
    if(isset($_SESSION["user"])){
        if(($_SESSION["user"])=="" or $_SESSION['usertype']!='p'){ // Changed 'd' to 'p' to match patient context
    ...
    // Changed query to use patient table and pemail
    $userrow = $database->query("select * from patient where pemail='$useremail'"); 
    $userfetch=$userrow->fetch_assoc();
    $userid= $userfetch["pid"]; // Changed docid to pid
    $username=$userfetch["pname"]; // Changed docname to pname
                    ...
                    <td>
                        <p style="font-size: 23px;padding-left:12px;font-weight: 600;">Settings</p>
                            ...
                            <td width="15%">
                                ...
                                // These queries seem out of place in patient settings, 
                                // but kept them as they were in the original code.
    ...
    if($_GET){
        ...
        if($action=='drop'){
            ...
            $sqlmain= "select * from patient where pid=?"; // Changed table to patient
            $stmt = $database->prepare($sqlmain);
            $stmt->bind_param("i", $id);
            $stmt->execute();
            $result = $stmt->get_result();
            $row=$result->fetch_assoc();
            $name=$row["pname"]; // Changed docname to pname
            $email=$row["pemail"]; // Changed docemail to pemail
            $address=$row["paddress"]; // Added address
            
            // Removed specialty related code as patients don't have it
            $dob=$row["pdob"]; // Added DOB
            $nic=$row['pnic']; // Changed docnic to pnic
            $tele=$row['ptel']; // Changed doctel to ptel
            echo '
            <div id="popup1" class="overlay">
                    <div class="popup">
                    <center>
            ...
            // Removed specialty related code
                    ...
                    <div id="popup1" class="overlay">
                            ...
                            <center>
                                ...
                                <tr>
                                    ...
                                    <tr>
                                        <td class="label-td" colspan="2">
                                            <form action="edit-user.php" method="POST" class="add-new-form"> <label for="Email" class="form-label">Email: </label>
                                    ...
                                    <tr>
                                        <td class="label-td" colspan="2">
                                            <input type="text" name="name" class="input-text" placeholder="User Name" value="'.$name.'" required><br> </td>
                                    ...
                                    <tr>
                                        <td class="label-td" colspan="2">
                                            <label for="spec" class="form-label">Address</label> </td>
                                    </tr>
                                    <tr>
                                        <td class="label-td" colspan="2">
                                        <input type="text" name="address" class="input-text" placeholder="Address" value="'.$address.'" required><br> </td>
                                    </tr>
                                    <tr>
                                        <td class="label-td" colspan="2">
